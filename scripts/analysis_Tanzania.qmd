---
title: "Data Analysis and Visualization <br> Tanzania Community Data"
author: Gagan Atreya
format:
  html:
    toc: true
...
ds_csv <- fread("~/Desktop/oxford/data/tanzania/Tanzania Data Entry - Sheet1.csv")

## XLSX file as the dataset:
ds_xls <- read_excel("~/Desktop/oxford/data/tanzania/Tanzania Data Entry.xlsx")
...
identical(ds_csv, ds_xls) 

all.equal(ds_csv, ds_xls)

cmp <- comparedf(ds_csv, ds_xls)
...
It seems that there are substantial differences between the xls file and the csv file.
I will be using the xls file for the analysis:
...
***Note: This needs a lot of work from someone familiar with Tanzanian education. The spreadsheet to edit can be found here:*** <br>

[Spreadsheet for cleaning: Tanzania](https://docs.google.com/spreadsheets/d/1wCSfFNaVlq8lvP_WO-vzyIlKo1cmwRbAd7uBSHGt7ok/edit?usp=sharing){target="blank"}
...
## Reliability <br> Scale 2: Experience of BBL
